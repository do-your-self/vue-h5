<template>
	<div class="vue-uploader">
		...
		<h3 class="file-tips">
            {{title}}
			<!-- {{maxlength - files.length}}张 -->
		</h3>
		<div class="file-list">
			<!-- img list -->
			<section v-for="(file, index) of files" class="file-item draggable-item" :key="index">
				<img :src="file.src" alt="" ondragstart="return false;" v-if="file.src">
...
export default {
  props: {
    ...
    change: {
      //修改数据源对象的值不能让数据源发生改变时使用（页面无刷新加载）
      type: Boolean,
      required: false
    },
    maxlength: Number,
  ...
  methods: {
    ...
    initFiles() {
      if (JSON.stringify(this.imgData) !== "{}") {
        for (let key in this.imgData) {
          var tempObj = {};
          tempObj["name"] = key;
          tempObj["src"] = this.imgData[key];
          tempObj["url"] = this.imgData[key];
    ...
    submit(file, index) {
      ...
      if (file.file) {
        this.axios.upload({
          ...
          if (data.success) {
            this.$set(file, "url", data.data); //生成预览地址
            if (this.maxlength === 1 || this.maxlength === 0) {
              this.$emit("input", data.data);
            } else {
              let imgUrls = "";
              for (let i =0;i<this.files.length;i++){
                i===this.files.length-1?imgUrls += this.files[i].url:imgUrls += this.files[i].url + '%26'
              }
    ...
    remove(list, index) {
      ...
      let imgUrls = "";
      for (let i =0;i<this.files.length;i++){
        i===this.files.length-1?imgUrls += this.files[i].url:imgUrls += this.files[i].url + '&'
      }
      this.$emit("input", imgUrls)
    ...
    async fileChanged() {
      ...
      if (this.files.length + list.length <= this.maxlength) {
        ...
        this.$toast({
          message: `最多选择${this.maxlength}张图片`,
          position: 'model',
          duration: 3000
        })
      }
